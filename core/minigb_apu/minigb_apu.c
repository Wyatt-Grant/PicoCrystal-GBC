...
/* PicoCrystal: for __not_in_flash_func() below -- minigb_apu_audio_read/write
 * are called on every APU register access (frequent whenever music/SFX is
 * playing) and were confirmed (via a linker-generated veneer jumping from
 * RAM back into flash to reach them) to still be flash-resident even after
 * their caller, __gb_write, was moved to RAM. */
#include "pico.h"
...
/* PicoCrystal: the per-sample synthesis path (audio_callback + the three
 * update_* generators) is RAM-resident too. It now runs on core1, and any
 * core1 flash instruction fetch contends with core0's constant XIP traffic
 * (2MB ROM reads + the emulator's remaining flash code) on the single QSPI
 * bus, so keeping the synthesis loops out of flash protects core0's
 * bandwidth as much as core1's own speed. */
static void __not_in_flash_func(update_square)(struct minigb_apu_ctx *ctx, audio_sample_t *samples,
		const bool ch2)
{
...
static void __not_in_flash_func(update_wave)(struct minigb_apu_ctx *ctx, audio_sample_t *samples)
{
...
static void __not_in_flash_func(update_noise)(struct minigb_apu_ctx *ctx, audio_sample_t *samples)
{
 ...
 */
void __not_in_flash_func(minigb_apu_audio_callback)(struct minigb_apu_ctx *ctx,
 ...
 */
uint8_t __not_in_flash_func(minigb_apu_audio_read)(struct minigb_apu_ctx *ctx, const uint16_t addr)
{
 ...
 */
void __not_in_flash_func(minigb_apu_audio_write)(struct minigb_apu_ctx *ctx,
